...
/**
 * This is a clone of DeflaterOutputStream, modified to work with a FastDeflater.
 *
 ...
 */
public class FastDeflaterOutputStream extends FilterOutputStream {
    /**
     * Compressor for this stream.
     */
    protected FastDeflater def;
     ...
     */
    public FastDeflaterOutputStream(OutputStream out, FastDeflater def, int size) {
     ...
     * @param def the compressor ("deflater")
     */
    public FastDeflaterOutputStream(OutputStream out, FastDeflater def) {
     ...
     * @since 1.7
     */
    public FastDeflaterOutputStream(OutputStream out, boolean syncFlush) {
        this(out, new FastDeflater(), 512, syncFlush);
    ...
    public void flush() throws IOException {
        if (syncFlush && !def.finished()) {
            int len = 0;
            while ((len = def.deflate(buf, 0, buf.length, FastDeflater.SYNC_FLUSH)) > 0)
            {
                out.write(buf, 0, len);
                if (len < buf.length)
                    break;
